...
class Database:
    ...
    def create_tables(self):
        ...
        """)
        # ATUALIZADO: Presença agora tem manha e tarde
        self.cursor.execute("""
    ...
    def migrate_tables(self):
        ...
        # NOVA MIGRAÇÃO: Presença por Turnos
        if not self.check_column_exists("presenca", "manha"):
            print("Migrando: Adicionando turnos à presença...")
            ...
            # Converte dados antigos: Se 'presente' era 1, assume dia cheio (manha=1, tarde=1)
    ...
    # --- MÉTODOS DE PRESENÇA (ATUALIZADOS) ---
    def salvar_presenca(self, func_id, data, manha, tarde):
        # Salva manha=1 e tarde=1 separadamente
        self.cursor.execute("""
    ...
    def get_presenca_dia(self, obra_id, data):
        # Retorna um dicionario {func_id: {'m': 1/0, 't': 1/0}}
        self.cursor.execute("""
    ...
    def relatorio_periodo(self, obra_id, d1, d2):
        # SOMA DIAS: (Soma Manhã + Soma Tarde) / 2
        # Ex: Manhã(1) + Tarde(0) = 1 / 2 = 0.5 dia
        self.cursor.execute("""
...
class EmployeeManager(QWidget):
    def __init__(self, db, obra_id):
        ...
        # Redimensiona M e T para ficarem pequenos
        self.tb.setColumnWidth(10, 30) # M
        self.tb.setColumnWidth(11, 30) # T
    ...
    def ld(self):
        ...
        for r, d in enumerate(fs):
            ...
            # Se tem registro, usa o valor. Se não tem, padrão é desmarcado (0)
...
class StockControl(QWidget):
    def __init__(self, db, obra_id):
        ...
        self.f_cat.currentTextChanged.connect(self.load_history) # Conecta o filtro de categoria
        
        filter_layout.addWidget(self.f_item)
        filter_layout.addWidget(self.f_cat) # Adiciona no layout
        ...
        self.in_origem = QLineEdit(); self.in_origem.setPlaceholderText("Origem") # Renomeado
        self.in_dest = QLineEdit(); self.in_dest.setPlaceholderText("Destino") # Novo
        ...
        gl2.addWidget(self.in_origem); gl2.addWidget(self.in_dest) # Adicionados novos campos
    ...
    def mov(self, t):
        ...
        try: 
            ...
            self.db.movimentar_estoque(self.sid, float(self.in_q.text().replace(',','.')), t, data_iso, self.in_origem.text(), self.in_dest.text(), self.in_nf.text()); self.ref()
        except: pass
    ...
    def load_history(self):
        ...
        for r,d in enumerate(his):
            ...
            self.tb_h.setItem(r,2,QTableWidgetItem(d[2])) # Item
            self.tb_h.setItem(r,3,QTableWidgetItem(d[3] or "-")) # Categ
            ...
            self.tb_h.setItem(r,5,QTableWidgetItem(f"{d[5]} {d[6]}")) # Qtd+Unid
            self.tb_h.setItem(r,6,QTableWidgetItem(d[7] or "")) # Origem
            self.tb_h.setItem(r,7,QTableWidgetItem(d[8] or "")) # Destino
            self.tb_h.setItem(r,8,QTableWidgetItem(d[9] or "")) # NF <--- ADICIONADO
...
class ReportTab(QWidget):
    ...
    def copiar_area_transferencia(self):
        ...
        for r in range(rows):
            try:
                # Agora pode ser float (ex: 4.5 dias), então mantemos o texto original
...
# --- CLASSE MATERIAL CALCULATOR (ATUALIZADA) ---
class MaterialCalculator(QWidget): 
    def __init__(self):
        ...
        # --- CÁLCULO DE CONCRETO (TRAÇO) ---
        gb2 = QGroupBox("Cálculo de Concreto (Traço)"); g2 = QGridLayout()
        
        # Inputs de Traço CORRIGIDO COM LAYOUT HORIZONTAL
        ...
        # Container para os spinboxes do traço
        h_traco = QHBoxLayout()
        h_traco.addWidget(self.sp_c)
        h_traco.addWidget(self.sp_a)
        h_traco.addWidget(self.sp_b)
        
        g2.addWidget(QLabel("Traço (C:A:B):"), 0, 0)
        
        # Adiciona o layout horizontal na grade, ocupando 2 colunas para alinhar com os inputs abaixo
        g2.addLayout(h_traco, 0, 1, 1, 2)
...
if __name__ == "__main__":
    try:
        myappid = 'miiza.gestor.obras.v25'
